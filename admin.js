...
(async function initCustomerDetails(){
  ...
  async function fetchCustomer(id){
    ...
    // Investment Plans must represent real approved investments only. Rejected/pending
    // payment attempts stay in Transaction History and must never appear as an active plan.
    const plans=[...new Map(txs.filter(t=>(t.plan_name||t.plan_id)&&String(t.status||'').toLowerCase()==='approved').map(t=>{const p=catalog.find(x=>String(x.id)===String(t.plan_id)); return [String(t.plan_id||t.plan_name),{...t,earning_mode:t.earning_mode||p?.earning_mode,earning_value:t.earning_value??p?.earning_value,earning_frequency:t.earning_frequency||p?.earning_frequency,earning_periods:t.earning_periods??p?.earning_periods,monthly_payout_percent:t.monthly_payout_percent??p?.monthly_payout_percent}]})).values()];
  ...
  function customerHtml(d,mode='full'){
    ...
    const txLabel=t=>{
      const st=txStatus(t);
      if(t.withdrawal_id || String(t.note||'').toLowerCase().includes('withdraw')) return 'WITHDRAWAL';
      return t.type==='credit'?'CREDIT':'BUY';
    };
    const txStatus=t=>String(t.status||'').trim().toLowerCase();
    // Display rule is intentionally based on the business event + status:
    // Payment/deposit approved => positive green; rejected => negative red.
    // Withdrawal approved => negative red; rejected => red only (no balance sign).
    const isWithdrawalTx=t=>!!t.withdrawal_id || String(t.note||'').toLowerCase().includes('withdraw');
    const txClass=t=>{
      const st=txStatus(t);
      if(isWithdrawalTx(t)) return 'debitText';
      if(st==='rejected') return 'debitText';
      if(t.type==='credit') return 'creditText';
      if(st==='approved') return 'creditText';
      return 'neutralText';
    };
    const txSign=t=>{
      const st=txStatus(t);
      if(isWithdrawalTx(t)) return st==='approved'?'−':'';
      if(st==='rejected') return '−';
      if(t.type==='credit') return '+';
      if(st==='approved' && (t.plan_id||t.plan_name)) return '+';
      return '−';
    };
    const txStatusLabel=t=>txStatus(t)==='approved'?'APPROVED':txStatus(t)==='rejected'?'REJECTED':txStatus(t).toUpperCase()||'UNKNOWN';
