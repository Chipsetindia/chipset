...
(async function initCustomerDetails(){
  ...
  function customerHtml(d,mode='full'){
    ...
    const txClass=t=>String(t.status||'').toLowerCase()==='rejected'?'rejectedText':(t.type==='credit'?'creditText':'debitText');
    const txSign=t=>String(t.status||'').toLowerCase()==='rejected'?'':(t.type==='credit'?'+':'-');
